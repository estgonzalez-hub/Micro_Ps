...
module top(
		...
		counter #(22, 2000000, 1) segment_time (.clk  (int_osc),  .reset (reset), .enable (enable), .out (sel));
		counter #(24, 12000000) matrix_time(.clk  (int_osc),  .reset (reset), .enable (enable), .out (out));
		
		seven_segment  display(.s (dip_switch), .seg (seg)); 
		scanning scanner(.sel (out), .reset (1'b0), .enable(1'b1), .rows (row));
